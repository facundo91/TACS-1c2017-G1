...
public class Movie {
    ...
    private List<ActorEnPelicula> cast = new ArrayList<>();
    ...
    @Autowired
    private TmdbService tmdbService; //TODO Movie no deberia conocer los Service
...
//	}
    ...
    public Movie(JSONObject jsonMovie) throws JSONException, IOException {
        this.setJsonResponse(tmdbService.getResource2("movie", jsonMovie.getString("id")));
        this.setInfo();
    }

    public Movie(String id) {
        try {
            this.setJsonResponse(tmdbService.getResource2("movie", id));
            this.id = id;
...
//		this.setReviews();
    ...
    private void setImages() throws JSONException, IOException {
        JSONObject images = tmdbService.getResource2("movie", this.getId() + "/images");
...
//		JSONArray reviews = TmdbServiceImpl.getResource2("movie", this.getId() + "/reviews").getJSONArray("results");
...
//	}
    ...
    private void setCredits() throws JSONException, IOException {
        JSONArray cast = tmdbService.getResource2("movie", this.getId() + "/credits").getJSONArray("cast");
